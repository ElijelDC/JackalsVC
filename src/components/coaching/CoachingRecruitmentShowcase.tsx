...
import { CoachingApplicationButton } from "@/components/coaching/CoachingApplicationForm";
import { AnimateIn } from "@/components/motion/AnimateIn";
import { StaggerIn } from "@/components/motion/StaggerIn";
import {
  COACHING_BENEFITS,
  COACHING_LOOKING_FOR,
} from "@/lib/coaching-recruitment-config";
import { PUBLIC_PATHS } from "@/lib/public-paths";

export function CoachingRecruitmentShowcase() {
  return (
    <>
      <ShowcaseHero
        title="Opportunity"
        ...
        description={
          <>
            ...
            Jackals Volleyball is expanding and is looking for passionate,
            dedicated coaches to help develop our National League teams and
            support people across the volleyball community. If you love the game
            and want to make an impact, we want to hear from you.
          </>
        }
        action={
        ...
        <div className="relative mx-auto max-w-4xl px-4 text-center sm:px-6">
          <AnimateIn variant="pop-in">
            <p className="text-xs font-semibold uppercase tracking-widest text-jackals-red-light">
              Keen to coach with us?
            </p>
            <h2 className="mt-4 font-display text-2xl font-bold text-white sm:text-3xl">
              We&apos;d love to hear from you
            </h2>
            <p className="mx-auto mt-5 max-w-2xl text-sm leading-relaxed text-zinc-400 sm:text-base">
              Tell us a bit about yourself and your coaching background. It only
              takes a few minutes — we&apos;ll read what you send and get back
              to you by email.
            </p>
          </AnimateIn>
          <StaggerIn
            ...
            variant="pop"
          >
            <CoachingApplicationButton className="gap-2" />
          </StaggerIn>
        </div>
      </section>
      ...
      <div className="mx-auto max-w-7xl px-4 py-16 sm:px-6 sm:py-20 lg:px-8">
        <AnimateIn variant="blur-in" className="mb-12 text-center sm:mb-14">
          ...
          <h2 className="mt-3 font-display text-3xl font-bold text-white">
            Benefits of joining the staff
          </h2>
          <p className="mx-auto mt-4 max-w-2xl text-sm leading-relaxed text-zinc-400">
            Help build one of Dublin&apos;s most active volleyball clubs — with
        ...
        <StaggerIn
          ...
          variant="pop"
        >
          {COACHING_BENEFITS.map(({ icon: Icon, title, description }) => (
            <article
              key={title}
        ...
        <AnimateIn variant="spring-up" className="mt-12 sm:mt-16">
          <ShowcaseCtaBand
            ...
            title="Ready to coach with Jackals?"
            description="Tell us a bit about yourself through the quick form below — we'll be in touch soon."
          >
            <CoachingApplicationButton className="w-full gap-2 whitespace-nowrap sm:w-auto" />
          </ShowcaseCtaBand>
        </AnimateIn>
      </div>
    </>
