...
import {
  apiGet,
  apiPostForm,
  apiRemoveTrainingInvitePaymentProof,
} from "@/lib/client-api";
import { compressImageFileForUpload } from "@/lib/client-image-compress";

type TrainingInvitePaymentProofUploadProps = {
  token: string;
  proofId: string | null;
...
export function TrainingInvitePaymentProofUpload({
  token,
  proofId,
  onProofChange,
  disabled = false,
...
}: TrainingInvitePaymentProofUploadProps) {
  ...
  }, [forceReupload, onProofChange, proofId, token]);
  ...
  const submitProof = async () => {
    ...
    const result = await apiPostForm<{
      ...
      `/api/training-invites/${token}/payment-proof`,
      formData,
  ...
  const removeProof = async () => {
    ...
    const result = await apiRemoveTrainingInvitePaymentProof(token, proofId);
  ...
  return (
    <div className="space-y-4">
      {forceReupload ? (
        <div className="rounded-lg border border-amber-500/40 bg-amber-500/10 px-4 py-3 text-sm text-amber-100">
          <div className="flex items-start gap-2">
            ...
            <div>
              <p className="font-semibold text-amber-50">New receipt required</p>
      ...
      <p className="text-sm text-zinc-400">
        After paying, upload a screenshot of your payment confirmation. You can
        register once this is uploaded.
      </p>
